      ...
      exitCode <- DatabaseAuthService.create(config).use { authService =>
        for {
          ...
          _ <- listResult match {
            ...
            case Left(error) =>
              IO(println(s"❌ Failed to list clients: ${error.error} - ${error.error_description.getOrElse("No description")}"))
          }
          _ <- IO(println())
          ...
          _ <- createResult match {
            ...
            case Left(error) =>
              IO(println(s"❌ Failed to create test client: ${error.error} - ${error.error_description.getOrElse("No description")}"))
          }
          _ <- IO(println())
          ...
          _ <- findResult match {
            case Some(client) => IO(println(s"✅ Found client: ${client.client_name}"))
          ...
          _ <- deleteResult match {
            ...
            case Left(error) =>
              IO(println(s"❌ Failed to delete test client: ${error.error} - ${error.error_description.getOrElse("No description")}"))
          }
          
          _ <- IO(println())
          _ <- IO(println("🎉 Debug test completed!"))
          _ <- IO(println())
          _ <- IO(println("💡 If all tests passed, client creation should work during startup"))
          _ <- IO(println("💡 If any tests failed, that's where the startup issue is occurring"))
